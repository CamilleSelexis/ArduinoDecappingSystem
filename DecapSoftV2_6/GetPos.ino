...
long GetPos(){
  ...
  uint8_t vect[ly+1]; //length = 141
  vect[0] = floor(ly*0.5);
  for(int i = 1;i<=ly;i++){
    vect[i] = vect[i-1] + pow(-1,i)*i; //70, 70 + (-1)^i * i
  }
  float Result[10];
  uint8_t found = 0;
  uint8_t Pos[100];
  Result[0] = 0;
  float avg[ly]; //length = 140
  float mvt[ly-2*n]; //length = 136
  ...
  if (cam.grab(Pfb) == 0){
    ...
    // Cropavg - Compute the avg on each line in the cropped img
      uint8_t k = 0;
      ...
      //-----Compute moving avg on the line avg computed before
